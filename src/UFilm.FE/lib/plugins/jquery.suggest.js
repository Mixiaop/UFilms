									...
									if(arr==null){
										...
										if(arr.length>0){										    
											for(i=0;i<arr.length;i++){
												...
												var id_field='';
												if(me.idField!=null){
													id_field=' id_field="' + arr.data[i].id + '"';
												}

												var thumb="";
												if(me.thumbnail==true){
													var style="";
													if(arr.data[i].thumbnail!=undefined){
														style=' style="background:url(' + arr.data[i].thumbnail + ') no-repeat;"';
													}
												...
												if(me.description==true){
													if(arr.data[i].description!=undefined){
														desc='<div class="description">' + arr.data[i].description + '</div>';
													}
												}

												html+='<div id="' + suggestRow + (i+1) + '" class="' + cssClass + '"' + id_field + ' seq_id="' + i + '" >' + thumb + '<div class="suggestion_title" relID="'+arr.data[i].id+'">' + arr.data[i].data.replace(new RegExp('(' + me.textField.val() + ')', 'gi'), "<b>$1</b>") + '</div>' + desc + '</div>';
											}
											...
											for(i=1;i<=arr.length;i++){
												...
												target.click(function(e){
													...
													if(me.onSelected!=null){
														me.onSelected.call(this, me.arrData.data[$(this).attr("seq_id")]);
													}
			...
			if(me.holder.css("display")!="none"){
				...
				if(e.keyCode==40){
					...
					if(currRow<=rows){
						...
						//alert(target.html());
						me.textField.attr('relID',target.find(".suggestion_title").attr('relID'));
						if(me.idField!=null){
					...
					if(me.onSelected!=null){
						if(currRow>0){
							me.onSelected.call(this, me.arrData.data[currRow-1]);
						}
						else{
							me.onSelected.call(this, null);
						}
